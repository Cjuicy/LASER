...
### 2.5 普通引擎的局部误差会污染后续全局配准

`StreamingWindowEngine` 将 `scale_mask` 直接乘进 `working_window['local_points']`，又把该结果保存为下一窗口全局 Sim(3) 的输入。一个错误局部区域可能因此影响下一窗口的整体尺度与相机轨迹。

HART 路径必须把逻辑状态分为：

- Base geometry：只完成窗口级 Sim(3)，供下一窗口全局配准；
- Refined geometry：Base geometry 乘 HART 局部 mask，供输出和局部锚点参考。

这只要求额外保留尾部 overlap 的 Base points 与一通道 local mask，不要求在内存中保存两份完整序列点云。
...
- 阻止 HART 局部 mask 进入下一窗口全局 Sim(3) 的 registration input。
...
不再执行“父节点融合后传子节点、子节点再融合后传下一帧”的逐跳过程。每个窗口重新估计自己的 TrackSegment 尺度；尺度不是绑定完整序列的永久常量。
...
不保存第二份三通道点云。`RegistrationState` 和 `AnchorPropagationState` 只保留尾部 overlap；完整窗口输出继续写缓存后释放。
...
下一窗口全局注册只读取 `RegistrationState.base_points_tail`，绝不读取 refined output。下一窗口局部锚点读取 base tail × local mask。
...
在线处理时维护临时 cumulative Base state 供 HART 估计，但不把它覆盖进 LC raw cache。回环优化后：
...
- 状态：当前完整窗口加上一窗口尾部 overlap，约 `O((W+O)P)`，不依赖序列长度。
- 额外常驻大数组：一通道 local scale mask、紧凑 labels 和尾部 Base state；不保存第二份完整三通道点云。
...
- 普通 HART 全局 registration input 未乘 local mask；
- 普通输出正确使用 `base × local`；
