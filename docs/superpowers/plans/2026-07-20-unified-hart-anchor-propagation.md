...
- [ ] 测试覆盖：
  ...
  - 当前完整窗口一次性赋值而非逐跳加权；
  - 下一窗口重新估计；
  - previous local mask 同时用于局部锚点和 propagated registration tail；
...
- [ ] HART 普通引擎保存：
  ...
  - 内存 Base tail = 未乘 local mask 的 Base points，防止数值重复；
  - 内存 propagated registration tail = `Base × local`，供下一窗口 Sim(3)；
  - 内存 anchor tail = local mask + segmentation/conf/track；
  - 不保存第二份完整窗口三通道 points。
- [ ] `_reset_state()` 清理两类新 state 和 propagator 运行状态。
- [ ] 测试使用三个合成窗口和 monkeypatch global registration，证明：
  - 下一窗口 global register 收到上一窗口 refined tail；
  - HART 尺度会改变后续 Sim(3) 和 camera poses；
...
- [ ] LC 的 HART 在线路径维护 cumulative Base/pose state 用于局部锚点，并用 `raw × local` tail 估计下一 pairwise Sim(3)；磁盘缓存仍保留 raw local points、raw poses 和 pairwise `sim3`，供 loop optimizer 使用。
...
| 在线状态 | 只保留尾部 overlap |
| 普通引擎 | 下一 registration 使用上一 Refined，输出使用 Base×Local |
